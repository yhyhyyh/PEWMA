...
def calculate_ARL(rep,c,T,tau,theta0,shift,Lambda,p,hp):
    ...
    for v in range(rep):
        ...
        for t in range(1, T + 1):
            ...
            if t <= tau:
                ...
                x[:, t] = np.random.negative_binomial(1 / c, 1 / (1 + c * (nt[:, t] * (theta0+shift))))
            theta[:, t] = x[:, t] / nt[:, t]
            if theta[:, t] < q[0]:
                Y[0] = 1
            elif theta[:, t] > q[p - 2]:
    ...
    print('SDRL=', SDRL)
    return ARL
# print('ARL=', calculate_ARL(rep=2000,c=0.05,T=1000,tau=50,theta0=1,shift=0.2,Lambda=0.05,p=5,hp=4.3636322021484375))
...
# 4.70703125 ARL0 200
# 6.807861328125 ARL0 500

# 4.3636322021484375 p=5 lambda=0.05
# 4.997243881225586 p=5 lambda=0.15
# 4.999995231628418 p=5 lambda=0.2
# 5.6250762939453125 p=5 lambda=0.25
# 5.234336853027344 p=5 lambda=0.25

# 0.6871185302734375 p=2
# 2.6305389404296875 p=4